;;; -*- lexical-binding: t -*-
;;; config-haskell.el --- FIRE ALL MONAD TRANSFORMERS
...
;;   (bind-keys :map haskell-mode-map
;;              ("C-c C-," . config-haskell/run-test-suite))
...
;;     (config/font-lock-replace-symbol 'haskell-mode "\\(->\\)" "→")
;;     (config/font-lock-replace-symbol 'haskell-mode "\\(<-\\)" "←")
;;     (config/font-lock-replace-symbol 'haskell-mode "\\(=>\\)" "⇒")
...
;; (defun config-haskell/run-test-suite ()
;;   (interactive)
;;   (require 'compile)
